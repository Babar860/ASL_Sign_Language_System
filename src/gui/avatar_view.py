...
class AvatarView(tk.Frame):
    ...
    def __init__(self, parent: tk.Misc, controller: Any, config: dict[str, Any]) -> None:
        ...
        tk.Button(input_row, text="Pause", command=lambda: self.avatar.pause()).pack(side="left", padx=6)
        tk.Button(input_row, text="Resume", command=lambda: self.avatar.resume()).pack(side="left", padx=6)
        tk.Button(input_row, text="Replay", command=lambda: self.avatar.replay(self._after)).pack(side="left", padx=6)
        ...
        self.avatar = Avatar(config, self._display_image, self._set_overlay)
